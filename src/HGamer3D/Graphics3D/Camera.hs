...
instance Serialise Camera where
    encode (FullViewCamera) = encode (0::Int) 
    encode (OverlayCamera v1 v2) = encode (1::Int) <> encode v1<> encode v2
    decode = do
...
instance Serialise Frustum where
    encode (Frustum v1 v2 v3) = encode v1 <> encode v2 <> encode v3
    decode = Frustum <$> decode <*> decode <*> decode
