...
int hg3durho3d0_register_msg_receiver(uint64_t  idItemType, uint64_t idEvtType, void* p, msgFP f)
{
  if (idItemType == id_inputeventhandler && idEvtType == id_mouseevent) {
    ((InputEventHandler*)p)->registerMouseEventFunction(f);
    return OK;
  }
  
  if (idItemType == id_inputeventhandler && idEvtType == id_keyevent) {
    ((InputEventHandler*)p)->registerKeyEventFunction(f);
    return OK;
  }
 
  if (idItemType == id_edittext && idEvtType == id_edittext) {
    ((EditTextItem*)p)->registerTextEventFunction(f);
    return OK;
  }
  if (idItemType == id_checkbox && idEvtType == id_checkbox) {
    ((CheckBoxItem*)p)->registerToggledEventFunction(f);
    return OK;
  }
  if (idItemType == id_button && idEvtType == id_button) {
    ((ButtonItem*)p)->registerPressedReleasedFunction(f);
    return OK;
  }
  if (idItemType == id_slider && idEvtType == id_slider) {
    ((SliderItem*)p)->registerSliderEventFunction(f);
    return OK;
  }
  if (idItemType == id_dropdownlist && idEvtType == id_dropdownlist) {
    ((DropDownListItem*)p)->registerSelectionEventFunction(f);
    return OK;
  }
}
